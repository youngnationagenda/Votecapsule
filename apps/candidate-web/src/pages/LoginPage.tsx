    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-gray-100 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          <div className="inline-flex items-center justify-center w-14 h-14 bg-amber-500 rounded-xl mb-4 shadow-lg"><User className="w-7 h-7 text-white" /></div>
          <h1 className="text-2xl font-bold text-gray-900">Vote Capsule™</h1>
          <span className="inline-flex items-center mt-1.5 px-2.5 py-0.5 rounded text-xs font-semibold bg-amber-100 text-amber-700">Candidate Portal</span>
          <p className="text-sm text-gray-500 mt-2">Access restricted to registered candidates</p>
        </div>
          ...
          {!mfaRequired ? (
            <form onSubmit={handleLogin} noValidate>
              ...
              <div className="space-y-4">
                ...
                <div><label className="vc-label">Password</label>
                  <div className="relative"><input className="vc-input pr-10" type={showPassword ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} required autoComplete="current-password" />
            ...
            <form onSubmit={handleMfa} noValidate>
              <h2 className="text-lg font-semibold text-gray-900 mb-4">MFA Verification</h2>
              {error && <div className="flex items-center gap-2 p-3 mb-4 rounded-md bg-red-50 border border-red-200"><AlertCircle className="w-4 h-4 text-red-500 flex-shrink-0" /><p className="text-sm text-red-700">{error}</p></div>}
        ...
        <p className="text-center text-xs text-gray-400 mt-6">Vote Capsule Technologies — Candidate Access Only</p>
      </div>
    </div>
  );
}
