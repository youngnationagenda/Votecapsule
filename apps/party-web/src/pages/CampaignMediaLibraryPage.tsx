...
const MEDIA_CATEGORIES = [
  ...
  { code: 'campaign_photo',    label: 'Campaign Photos',     icon: Camera,      color: 'text-violet-600',   bg: 'bg-violet-50' },
  ...
  return (
    <div
      className={`border-2 border-dashed rounded-2xl p-8 text-center transition-all cursor-pointer ${
        dragging ? 'border-amber-400 bg-violet-50' : 'border-gray-200 hover:border-violet-300 hover:bg-violet-50/30'
      }`}
        ...
        >
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
            <button onClick={(e) => { e.stopPropagation(); getUrl().then((u) => { if (u) { setUrl(u); setPreview(true); } }); }}
              className="p-2 bg-white rounded-xl shadow text-gray-700 hover:text-violet-600">
...
// ── Main Page ─────────────────────────────────────────────────
function CampaignMediaLibraryContent(): React.JSX.Element {
  const qc       = useQueryClient();
...
export function CampaignMediaLibraryPage() {
  return (
    <PageErrorBoundary page="Campaign Media Library">
      <CampaignMediaLibraryContent />
    </PageErrorBoundary>
  );
}
