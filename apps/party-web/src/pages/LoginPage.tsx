...
export function LoginPage(): React.JSX.Element {
  ...
  const handleLogin = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch(loginSuccess({ user: { id: '', email, roles: ['PARTY_ADMIN'] }, accessToken: result.accessToken }));
      navigate('/dashboard');
    } catch { dispatch(loginFailure('Invalid email or password')); }
  ...
  const handleMfa = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      dispatch(loginSuccess({ user: { id: '', email, roles: ['PARTY_ADMIN'] }, accessToken: (data.data ?? data).accessToken }));
      navigate('/dashboard');
              ...
              <div className="space-y-4">
                ...
                <div><label className="vc-label">Password</label>
                  <div className="relative">
                    <input className="vc-input pr-10" type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="current-password" />
