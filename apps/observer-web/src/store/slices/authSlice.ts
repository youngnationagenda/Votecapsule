...
interface AuthUser { id: string; email: string; roles: string[]; organization?: string; accessLevel?: string; }
...
const authSlice = createSlice({ name: 'auth', initialState, reducers: {
  ...
  logout(state) { state.isAuthenticated = false; state.user = null; state.accessToken = null; state.isLoading = false; state.error = null; localStorage.removeItem('vc_observer_token'); localStorage.removeItem('vc_observer_user'); },
}});
